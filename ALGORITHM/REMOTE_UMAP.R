# Testing UMAP on some variables within the data
# sessionInfo()
...
CEPRUmap <- select(chosenVariables, -female) %>%
  as.matrix()
...
for (n_n in n_neighbors) {
  ...
  for (m_d in min_dist) {
    ...
    cpsTibUmap <- chosenVariables %>% 
      ...
      gather(key = "Variable", value = "Value", c(-UMAP1, -UMAP2, -female))
    
    existingCPSTibUmapPlot <- ggplot(cpsTibUmap, aes(UMAP1, UMAP2, col = female)) +
      facet_wrap(~Variable) +
      geom_point(size = 3) + 
      theme_bw()
